        ...
        for c, p in zip(correct_slot, pred_slot):
            ...
            if inCorrect is True:
                if __endOfChunk(lastCorrectTag, correctTag, lastCorrectType,
                                correctType) is True and __endOfChunk(
                                    lastPredTag, predTag, lastPredType,
                                    predType) is True and (lastCorrectType
                                                           == lastPredType):
                    inCorrect = False
                    correctChunkCnt += 1.0
                    if lastCorrectType in correctChunk:
                        ...
                        correctChunk[lastCorrectType] = 1.0
                                  ...
                                  correctType) != __endOfChunk(
                                      lastPredTag, predTag, lastPredType,
                                      predType) or (correctType != predType):
                    inCorrect = False

            if __startOfChunk(lastCorrectTag, correctTag, lastCorrectType,
                              correctType) is True and __startOfChunk(
                                  lastPredTag, predTag, lastPredType,
                                  predType) is True and (correctType
                                                         == predType):
                inCorrect = True
...
class ExternalMetrics(object):
    ...
    def accuracy(pred_list, real_list):
        ...
        return (pred_arr == real_arr).sum() * 1.0 / len(pred_arr)
